...
import { useGroupedMessagesStream } from '@/app/autoaction/hooks/useGroupedMessages';

export default function ConversationList() {
  const router = useRouter();
  const { conversations, loading, error } = useGroupedMessagesStream();
    ...
    <div className="space-y-4">
      {conversations.length === 0 ? (
        ...
        conversations.map((conversation) => (
            <div
              key={conversation.id || `${conversation.contact}-${conversation.messages[0]?.id ?? crypto.randomUUID()}`}
            ...
            <p><strong>Contato:</strong> {conversation.name || conversation.contact}</p>
